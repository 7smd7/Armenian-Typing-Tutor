...
const TypingHand: React.FC<TypingHandProps> = ({ side, fingerToHighlight }) => {
    const shouldHighlight = (finger: 'Thumb' | 'Index' | 'Middle' | 'Ring' | 'Pinky'): boolean => {
        if (!fingerToHighlight) return false;
        if (fingerToHighlight === 'BothThumbs' && finger === 'Thumb') return true;
    ...
    return (
        <div className="hidden lg:block relative w-32 h-48 flex-shrink-0">
            <svg
                viewBox="0 0 100 135"
                className={`absolute inset-0 w-full h-full ${side === 'right' ? 'transform -scale-x-100' : ''}`}
                aria-hidden="true"
            >
                <path
            ...
            {Object.entries(highlightPositions).map(([finger, [top, left]]) =>
                shouldHighlight(finger as any) && (
                    <div
                        ...
                        style={{
                            top,
                            left: side === 'right' ? `calc(100% - ${left})` : left,
